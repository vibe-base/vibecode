const express = require('express');
const router = express.Router();

// Mock projects data
const MOCK_PROJECTS = [
  {
    id: 'mock-project-1',
    name: 'Mock Project 1',
    description: 'This is a mock project for development',
    language: 'Python',
    owner_id: 'mock-user-id',
    members: ['mock-user-id'],
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString()
  },
  {
    id: 'mock-project-2',
    name: 'Mock Project 2',
    description: 'Another mock project for development',
    language: 'JavaScript',
    owner_id: 'mock-user-id',
    members: ['mock-user-id'],
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString()
  }
];

// Log all requests
router.use((req, res, next) => {
  console.log('Request received:', {
    method: req.method,
    url: req.originalUrl,
    headers: req.headers,
    query: req.query,
    body: req.body
  });
  next();
});

// Special handler for /api/projects endpoint
router.get('/api/projects', (req, res) => {
  console.log('Direct handler for /api/projects called');
  console.log('Headers:', req.headers);
  console.log('Query:', req.query);
  
  // Return mock projects data
  return res.json(MOCK_PROJECTS);
});

// Catch-all route for any other endpoint
router.all('*', (req, res, next) => {
  console.log('Catch-all route hit:', req.method, req.originalUrl);
  
  // If this is a projects endpoint, handle it
  if (req.originalUrl.includes('/projects')) {
    console.log('Projects endpoint detected in catch-all');
    
    // Handle different HTTP methods
    if (req.method === 'GET') {
      if (req.originalUrl.includes('/api/projects/')) {
        // Get a specific project
        const projectId = req.originalUrl.split('/').pop();
        const project = MOCK_PROJECTS.find(p => p.id === projectId);
        
        if (!project) {
          return res.status(404).json({ error: 'Project not found' });
        }
        
        return res.json(project);
      } else {
        // List all projects
        return res.json(MOCK_PROJECTS);
      }
    } else if (req.method === 'POST') {
      // Create a new project
      const newProject = {
        id: `mock-project-${Date.now()}`,
        name: req.body.name || 'New Project',
        description: req.body.description || 'A new project',
        language: req.body.language || 'Unknown',
        owner_id: 'mock-user-id',
        members: ['mock-user-id'],
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      };
      
      return res.status(201).json(newProject);
    } else if (req.method === 'PUT' || req.method === 'PATCH') {
      // Update a project
      const projectId = req.originalUrl.split('/').pop();
      const project = MOCK_PROJECTS.find(p => p.id === projectId);
      
      if (!project) {
        return res.status(404).json({ error: 'Project not found' });
      }
      
      const updatedProject = {
        ...project,
        name: req.body.name || project.name,
        description: req.body.description || project.description,
        language: req.body.language || project.language,
        updated_at: new Date().toISOString()
      };
      
      return res.json(updatedProject);
    } else if (req.method === 'DELETE') {
      // Delete a project
      const projectId = req.originalUrl.split('/').pop();
      const project = MOCK_PROJECTS.find(p => p.id === projectId);
      
      if (!project) {
        return res.status(404).json({ error: 'Project not found' });
      }
      
      return res.status(204).send();
    }
  }
  
  // Continue to the next middleware
  next();
});

module.exports = router;
